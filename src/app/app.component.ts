...
export class AppComponent implements OnInit, OnDestroy {
	...
	public ngOnInit(): void {
		try {
			...
			this._currentUserSub = this.auth.currentUser$.subscribe((user: User) => this.currentUser = user);
			this._authStatusSub = this.auth.status$.subscribe((status: AUTH_STATUS) => this.authStatus = status);
		} catch (err) {
